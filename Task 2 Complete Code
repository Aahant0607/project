...
def localize_dataset(cfg, num_samples=3000):
    ...
    # ⭐ FIX: Added a retry loop to handle network errors during initial load
    max_retries = 3
...
if __name__ == '__main__':
    ...
    if model_path.exists():
        print(f"🗑️ Deleting old model checkpoint: {model_path}")
        model_path.unlink()
    else:
        print("👍 No old model checkpoint found. Starting fresh.")
    
    # Manually define the absolute paths for your Kaggle environment(When STEP 1: Comment out the localization process to prevent re-downloading)
